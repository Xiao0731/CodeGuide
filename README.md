...
| 维度                  | 权重 | 含义                                   |
| --------------------- | ---: | -------------------------------------- |
| Problem Understanding |  20% | 是否正确理解题意、输入输出与约束       |
| Algorithm Explanation |  30% | 是否解释核心算法、关键观察与正确性     |
| Reasoning Flow        |  20% | 是否由直观或朴素思路自然推导到最终方案 |
| Code Alignment        |  20% | 最终代码是否与讲解的算法和接口一致     |
| Beginner Friendly     |  10% | 表达是否清楚、是否适合初学者理解       |
...
| 对比         |  原始胜率 | 非平局胜率 |
| ------------ | --------: | ---------: |
| SFT vs Base  | **55.0%** |  **63.2%** |
| GRPO vs Base | **38.0%** |  **58.5%** |
| GRPO vs SFT  | **58.0%** |  **52.3%** |

##### 分维度结果

SFT 对 Base 的教学偏好较明确；GRPO 仍优于 Base，但没有在整体教学偏好上完全超过 SFT。双 Judge 的五维平均绝对分如下：

| 维度       |   Base |        SFT |       GRPO | 最优阶段 |
| ---------- | -----: | ---------: | ---------: | -------- |
| 题意理解   | 5.8800 |     5.9225 | **5.9950** | GRPO     |
| 算法讲解   | 5.0575 |     5.1225 | **5.3100** | GRPO     |
| 推导流程   | 5.4750 |     5.5175 | **5.5900** | GRPO     |
| 代码一致性 | 5.4700 |     5.5650 | **5.7400** | GRPO     |
| 初学者友好 | 5.2575 | **5.5550** |     5.4200 | SFT      |
